...
#include <poll.h>
#include <errno.h>
#include <unistd.h>  // close() supports
#include <pthread.h>
#include <sys/wait.h>
#include <signal.h>
...
int setfmtCamera()
{
	...
	printf("width = %d,height = %d ,fmt_width=%d,fmt_heigth=%d\n",width,height,format.fmt.pix.width,format.fmt.pix.height);
	format.fmt.pix.pixelformat = V4L2_PIX_FMT_SRGGB8;  // 设置为yuyv格式数据   V4L2_PIX_FMT_SRGGB8
...
int initmmap()
{
	...
	for(i =0; i < reqbuf.count; i++){
		...
		printf("start:%x  length:%d\n",(unsigned long)framebuf[i].start,framebuf[i].length);
	}
	return 0;
}
